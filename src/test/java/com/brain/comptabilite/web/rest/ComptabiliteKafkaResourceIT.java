package com.brain.comptabilite.web.rest;
...
import com.brain.comptabilite.config.KafkaProperties;
...
class ComptabiliteKafkaResourceIT {
    ...
    void setup() {
        ...
        ComptabiliteKafkaResource kafkaResource = new ComptabiliteKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/comptabilite-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/comptabilite-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
